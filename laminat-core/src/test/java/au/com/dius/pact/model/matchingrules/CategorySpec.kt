...
class CategorySpec : StringSpec({
        ...
        forAll(
            table(
                ...
                row(
                    PactSpecVersion.V2,
                    mapOf<String, Any>(
                        "$.body[0]" to mapOf("match" to ("type"), "max" to (5)),
                        "$.body[0][*].id" to mapOf("match" to ("regex"), "regex" to ("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}"))
                    )
                ),
                row(
                    PactSpecVersion.V3,
    ...
    "writes header matchers in the correct format" {
        ...
        category.toMap(PactSerializationConfig(PactSpecVersion.V2, null)) shouldBe mapOf(
            "$.headers.Content-Type" to mapOf("match" to "regex", "regex" to "application/json;\\s?charset=(utf|UTF)-8")
        )
        category.toMap(PactSerializationConfig(PactSpecVersion.V3, null)) shouldBe mapOf(
            "Content-Type" to mapOf("matchers" to listOf(mapOf("match" to "regex", "regex" to "application/json;\\s?charset=(utf|UTF)-8")), "combine" to "AND")
        )
    }
